              ...
              <?php if ($variable_dto->variableType == 'Dataschema'): ?>
                <ul>
                  <?php foreach ($variable_dto->studySummaries as $studySummary): ?>
                    <li>
                      <?php print l(mica_obiba_commons_get_localized_field($studySummary, 'name'), 'mica/study/' . $studySummary->id . '/' . mica_obiba_commons_to_slug(mica_obiba_commons_get_localized_field($studySummary, 'name'))); ?>
                    </li>
                  <?php endforeach ?>
                </ul>
              <?php elseif (!empty($variable_dto->studySummaries)): ?>
                <?php print l(mica_obiba_commons_get_localized_field($variable_dto->studySummaries[0], 'name'), 'mica/study/' . $variable_dto->studySummaries[0]->id . '/' . mica_obiba_commons_to_slug(mica_obiba_commons_get_localized_field($variable_dto->studySummaries[0], 'name'))); ?>
              <?php endif ?>
            </td>
          </tr>
        <?php endif; ?>
        ...
        <?php if (!empty($variable_dto->datasetId)): ?>
          <tr>
            ...
            <td>
              <p>
                <?php
                print l(mica_obiba_commons_get_localized_field($variable_dto, 'datasetName'), 'mica/' . mica_obiba_variable_dataset_type($variable_dto)
                  . '/' . $variable_dto->datasetId . '/' . mica_obiba_commons_to_slug(mica_obiba_commons_get_localized_field($variable_dto, 'datasetName')));
                ?>
              </p>
            </td>
          </tr>
            ...
            <p>
              ...
              <?php if ($variable_dto->variableType == 'Harmonized'): ?>
                ...
                print '(' . l($variable_dto->name, 'mica/variable/' . $variable_dto->datasetId . ':' . $variable_dto->name
                    . ':Dataschema' . '/' . mica_obiba_commons_to_slug($variable_dto->name)) . ')';
                ?>
              <?php endif; ?>
            </p>
          </td>
    ...
    <div class="col-lg-6 col-xs-12">
      ...
      <?php if (!empty($variable_dto->termAttributes)): ?>
        ...
        <?php foreach ($variable_dto->termAttributes as $termAttributes) : ?>
          <h4>
            <?php print mica_obiba_commons_get_localized_field($termAttributes->taxonomy, 'titles'); ?>
          </h4>
          <p class="help-block">
            <?php print mica_obiba_commons_get_localized_field($termAttributes->taxonomy, 'descriptions'); ?>
          </p>
          ...
          <table class="table table-striped">
            ...
            <?php foreach ($termAttributes->vocabularyTerms as $termAttribute) : ?>
              <tr>
                <td>
                  <h5>
                    <?php print mica_obiba_commons_get_localized_field($termAttribute->vocabulary, 'titles'); ?>
                  </h5>

                  <p class="help-block">
                    <?php print mica_obiba_commons_get_localized_field($termAttribute->vocabulary, 'descriptions'); ?>
                  </p>
                </td>
                ...
                <td>
                  <?php if (count($termAttribute->terms == 1)): ?>
                    <p data-toggle="tooltip"
                      title="<?php print mica_obiba_commons_get_localized_field($termAttribute->terms[0], 'descriptions'); ?>">
                      <?php print mica_obiba_commons_get_localized_field($termAttribute->terms[0], 'titles'); ?>
                    </p>
                  <?php else: ?>
                    <ul>
                      <?php foreach ($termAttribute->terms as $term) : ?>
                        <li data-toggle="tooltip"
                          title="<?php print mica_obiba_commons_get_localized_field($term, 'descriptions'); ?>">
                          <?php print mica_obiba_commons_get_localized_field($term, 'titles'); ?>
                        </li>
                      <?php endforeach; ?>
                    </ul>
                  <?php endif; ?>
...
<?php if (!empty($variable_dto->categories)): ?>
  <section>
    ...
    <?php print mica_obiba_variable_get_categories($variable_dto->categories); ?>
  </section>
<?php endif; ?>
...
<section>
  ...
  <div class="scroll-content-tab">
    <div id="param-statistics" var-id="<?php print $variable_dto->id; ?>">
      <div id="toempty">
        <img
          src="<?php print base_path() . drupal_get_path('theme', mica_obiba_commons_get_current_theme()) ?>/img/spin.gif">
      </div>
    </div>
...
<?php if ($variable_dto->variableType != 'Study'): ?>
  <section>
    ...
    <?php if ($variable_dto->variableType == 'Dataschema'): ?>
      <div class="scroll-content-tab">
        <?php print mica_obiba_variable_get_harmonizations($variable_dto); ?>
      </div>
