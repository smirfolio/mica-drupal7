<?php if (!empty($dataset)): ?>
  <div class="row sm-bottom-margin document-item-list flex-row">
    ...
    <div class="col-md-10  col-sm-12 col-xs-12">
      <h4>
        <?php
        $acronym = mica_obiba_commons_get_localized_field($dataset, 'acronym');
        $name = mica_obiba_commons_get_localized_field($dataset, 'name');
        print l($acronym == $name ? $acronym : $acronym . ' - ' . $name,
          'mica/' . mica_obiba_dataset_type($dataset) . '/' . $dataset->id); ?>
      </h4>
      <hr class="no-margin">
      <p class="md-top-margin">
        <small>
          <?php print empty($dataset->description) ? '' :
            truncate_utf8(strip_tags(mica_obiba_commons_get_localized_field($dataset, 'description')), 250, TRUE, TRUE);; ?>
        </small>
      </p>
  ...
  <?php
  $dataset_name = mica_obiba_commons_get_localized_field($dataset, 'name');
  ?>
<?php endif; ?>