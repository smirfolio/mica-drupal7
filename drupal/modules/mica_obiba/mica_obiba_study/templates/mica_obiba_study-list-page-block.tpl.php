...
<div class="row sm-bottom-margin document-item-list flex-row">
  ...
  <div class="col-md-10  col-sm-12 col-xs-12">
    <div>
      <h4>
        <?php
        $acronym = mica_obiba_commons_get_localized_field($study, 'acronym');
        $name = mica_obiba_commons_get_localized_field($study, 'name');
      ...
      <p class="md-top-margin">
        <small>
          <?php print truncate_utf8(strip_tags(mica_obiba_commons_get_localized_field($study, 'objectives')), 250, TRUE, TRUE); ?>
        </small>
      </p>
    </div>
    <ul class="search-item-list-no-style sm-top-margin help-block">
      <li>
        <?php foreach ($network_digests as $digest) {
          $names = array();
          array_push($names, mica_obiba_commons_get_localized_field($digest, 'name'));
        }
